...
export default function () {
    ...
    useEffect(function () {
        if (userDetails.username) {
            console.log("inif");
            fetch("http://localhost:3000/giveupcomingorderdetailstocity", {
                method: "POST",
    ...
    useEffect(function () {
        if (userDetails.username) {
            console.log("inif");
            fetch("http://localhost:3000/givereceivedorderdetailstocity", {
                method: "POST",
                headers: { "content-type": "application/json" },
                body: JSON.stringify({ userid: userDetails.userid }),
            }).then(function (response) {
    ...
    useEffect(function () {
        if (userDetails.username) {
            console.log("inif");
            fetch("http://localhost:3000/givedispatchedorderdetailstocity", {
                method: "POST",
                headers: { "content-type": "application/json" },
                body: JSON.stringify({ userid: userDetails.userid }),
            }).then(function (response) {
    ...
    function dispatchbutton(data) {
        return function () {
            fetch("http://localhost:3000/dispatchtouser", {
                method: "POST",
                headers: { "content-type": "application/json" },
                body: JSON.stringify({ orderid: data.orderid ,address:data.address,userid:userDetails.userid})
            }).then(function (response) {
                if (response.status == 500) {
                    Swal.fire({ title: "something went wrong", icon: "error" })
                }
    ...
    function Receivebutton(data) {
        return function () {
            fetch("http://localhost:3000/receivedorderatcity", {
                method: "POST",
                headers: { "content-type": "application/json" },
